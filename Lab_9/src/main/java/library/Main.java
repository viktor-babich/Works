...
public class Main {
    public static void main(String[] args) throws Exception {
        ...
        // register an email notifier observer
        lib.addObserver(new EmailNotifier());

        // add a one time anonymous observer that removes itself after first notification
        lib.addObserver(new OverdueObserver() {
            @Override
            public void notifyOverdue(User user, LibraryItem item, int daysLate) {
                System.out.println("One-time observer: " + user.getId() + " has overdue item " + item.getId());
    ...
    public static class Simulation {
        ...
        public Stats run(int days, double alphaBook, double alphaJournal, double alphaFilm, double beta) {
            ...
            for (int day = 1; day <= days; day++) {
                ...
                // notify observers about newly overdue items
                library.checkOverdues(currentDay);
                for (User u : users) {
                    java.util.List<LibraryItem> borrowed = new java.util.ArrayList<>(u.getBorrowed());
                    for (LibraryItem it : borrowed) {
                        boolean returned = false;
                        if (u.isOnTime()) {
                            int dueDate = it.borrowDate + ((Loanable)it).getLoanPeriod(u);
